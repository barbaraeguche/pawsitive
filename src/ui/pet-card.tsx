...
export default function PetCard({ pets, isAdopting }: {
	...
	return (
		<div className={'!mt-6 !mb-28 space-y-6 md:space-y-12'}>
			{/* default format */}
			<MobileFormat pets={pets} isAdopting={isAdopting}/>
			{/* desktop format */}
			<DesktopFormat pets={pets} isAdopting={isAdopting}/>
		</div>
	);
}
...
function AdjustTextSize({ name, value }: {
	...
	return (
		<p>
			...
			<span className={'text-[13.5px]'}>{capitalizeFirstLetter(value)}</span>
		</p>
	);
}

const MobileFormat = ({ pets, isAdopting }: {
	pets: Pet[],
	isAdopting?: boolean
}) => {
	return (
...
const DesktopFormat = ({ pets, isAdopting }: {
	pets: Pet[],
	isAdopting?: boolean
}) => {
	return (
